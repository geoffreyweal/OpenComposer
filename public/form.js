...
// hidden field: a template may emit "#SBATCH --ntasks=" for both a simple
// "Number of Cores" field and an advanced "Number of Tasks" field. A hidden
// field is therefore only auto-enabled from a line UNIQUE to it.
...
// Parse the scheduler directives in the script textarea back into the form
// widgets, using the patterns registered in ocForm.scriptLinePatterns by form.rb.
// This is the reverse direction of updateScriptContents(): it is what keeps the
// widgets in step when the user edits the script by hand.
//
...
// saved script so the form reconstructs its full state. The live two-way sync
// (typing in the editor) passes false: it updates only the widgets that are
// already active and never flips toggles or expands hidden sections.
ocForm.parseScriptToWidgets = function(allowEnable) {
  ...
  for (const pat of ocForm.scriptLinePatterns) {
    ...
    for (var i = 0; i < pat.keys.length; i++) {
      ...
      // A zero-padded capture is always digits (form.rb matched it with \d+),
      // so drop the padding: the field shows 4 rather than 04, and
      // zeropadding() re-applies it when the line is written back out.
      if (pat.zeroPad && pat.zeroPad[i]) value = String(parseInt(value, 10));

      switch (widget) {
      case 'number':
      case 'text':
      case 'email':
      // A path widget renders as a plain <input type="text" id="<key>">, with
      // the Select Path dialog only writing into it, so it reads back exactly
      // like a text field. Without this its line (e.g. "cd #{working_dir}")
      // would be silently rebuilt from the stale widget on the next patch.
      case 'path': {
        ...
        if (el && (!el.disabled || allowEnable)) {
          ...
          // ambiguous line (also emitted by another field) must not expand a
          // hidden section. The value is still applied, so if the section is
          // later opened by its own unique line it shows the right value.
      ...
      case 'checkbox': {
        ...
        while (true) {
          ...
          if (!optEl) break;
          var wanted = vals.indexOf(optEl.dataset.value) >= 0;
          if (!optEl.disabled) {
            // An active group mirrors the line exactly: a value the user deleted
            // from the script unticks its box instead of being re-added on the
            // next patch. Only reachable when the line was found, so a hidden
            // (never-emitted) line cannot clear the group.
            optEl.checked = wanted;
          }
          else if (wanted && allowEnable) {
            ocForm.autoEnable(key);
            optEl.checked = true;
          }
          n++;
...
// This replaces the old "regenerate the whole script and warn the user first"
// behaviour: a widget change now rewrites just the line that widget owns.
...
ocForm.patchScript = function() {
  ...
  // remembering the text each pattern actually matched so an edited literal line
  // can be preserved below.
  const currentLines = ocForm.scriptArea.value.split('\n');
  const consumed = new Set();
  const currentTextOf = new Map();
  const tempMarks = currentLines.map(function(line) {
    for (let pi = 0; pi < sortedPatterns.length; pi++) {
      const pfx = sortedPatterns[pi].prefix;
      if (pfx && line.startsWith(pfx) && !consumed.has(pi)) {
        consumed.add(pi);
        currentTextOf.set(pi, line);
        return pi;
      }
    }
    return null;
  ...
  for (let ni = 0; ni < newLines.length; ni++) {
    ...
    // A literal template line is driven by nothing but the user (no widget
    // feeds it), so if the script's version differs they edited it in place —
    // e.g. appending a flag to "srun ./a.out". Keep their text rather than
    // resetting it to the template's, which would silently discard the edit.
    let text = newLines[ni];
    if (pi !== undefined && sortedPatterns[pi].literal) {
      const cur = currentTextOf.get(pi);
      if (cur !== undefined && cur !== text) text = cur;
    }
    out.push(text);
  }
...
ocForm.addSelectedItem = function(id, triggerDynamic = true) {
  ...
  const addBadge = () => {
    ...
    badge.addEventListener('click', (event) => {
      ...
      const contentType = scriptOverwriteFlag ? 'script'
